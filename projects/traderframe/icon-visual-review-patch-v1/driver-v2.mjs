...
function selectedMarkups() {
  return Object.fromEntries(traderFrameCreativeLoopIcons.map(([id]) => [id, renderTraderFrameSelectedIcon(id)]));
}
...
export async function runTraderFrameIconVisualReviewPatchV2({ repoRoot, outputDir } = {}) {
  ...
  const sourceJobs = traderFrameCreativeLoopIcons.map(([name, title, semantic]) => ({
    ...
    version: '1',
    kind: 'icon-review-source',
    title: `${title} visual-review source`,
    projectId: 'traderframe',
    operation: 'generate',
    recipe: 'icon-system-recipe',
    requiredCapabilities: ['svg', 'vector', 'icon-master'],
    rights: { status: 'project-generated', legalReview: 'unresolved' },
    input: { svg: beforeMarkups[name], outputPath: `source-icons/${name}.svg`, requireFontFree: true, vectorOnly: true },
    metadata: { semantic, sourceCreativeLoop: 'traderframe-icon-creative-loop-v1', selectedDirectionId: 'gate-decision', sourceCandidate: true, userApproval: false }
  }));
  ...
  const finalAdapter = createLocalSvgAdapter({ rootDir: targetRoot });
  const finalJobs = traderFrameCreativeLoopIcons.map(([name, title, semantic]) => {
    const sourceArtifact = sourceBatch.artifacts.find((artifact) => artifact.id === `traderframe-${name}-visual-review-source-v1`);
    return {
      ...
      requiredCapabilities: ['svg', 'vector', 'icon-master'],
      rights: { status: 'project-generated', legalReview: 'unresolved' },
      source: { artifactRef: sourceArtifact?.ref, fileRef: sourceArtifact?.files?.[0]?.ref },
  ...
  const finalBatch = await executeProductionBatch({ jobs: finalJobs, assignments: finalAssignments, adapters: [finalAdapter] });
  ...
  const manifest = {
    ...
    projectId: 'traderframe',
    source: 'traderframe-icon-creative-loop-v1/gate-decision',
  ...
  const artifacts = [...sourceBatch.artifacts, ...finalBatch.artifacts, ...documentArtifacts, ...renderArtifacts];
  const graph = buildArtifactGraph(artifacts);
  const documentsPass = documentExecutions.every((execution) => execution.pass);
  const pass = sourceBatch.pass && finalBatch.pass && documentsPass && manifestExecution.pass && graph.pass && blockingAfter === 0 && renderArtifacts.length === traderFrameIconTargetSizes.length;
