...
export function runCreativeRuntime(input) {
  ...
  const styleFrameProof = buildStyleFrameProof({ exploration: creativeWorldExploration });
  ...
  const frameProofBlocked = styleFrameProof.findings?.some((item) => item.severity === 'blocker') === true;
  const worldSelected = selectedCreativeWorld?.selected === true && selectedCreativeWorld?.truth?.humanCreativeSelectionConfirmed === true;
  const status = !creativeThesis.pass || worldBlocked || frameProofBlocked
    ? 'blocked'
    : (creativeThesis.reviewReady === false || creativeWorldExploration.reviewReady === false || styleFrameProof.reviewReady === false || !worldSelected || creativeDirection.provisional)
      ? 'provisional'
      : 'ready-for-high-fidelity-style-frame-production';

  return {
    id: input.id,
    taskType: input.taskType,
    status,
    stages: ['inspiration', 'creative-thesis', 'creative-world', 'style-frame-proof', 'creative-direction', 'design', 'image', 'motion', 'creative-eval'],
    inspiration,
    creativeThesis,
...
export function validateBenchmark(output, expected) {
  ...
  if (expected.requireStyleFrameProof) {
    const proof = output.styleFrameProof;
    const worldCount = output.creativeWorldExploration?.worlds?.length ?? 0;
    if (!proof) failures.push('style frame proof is missing');
    else {
      if (proof.reviewReady !== true) failures.push('style frame proof plan is not browser-proof-ready');
      if ((proof.frames?.length ?? 0) !== worldCount * 5) failures.push(`style frame proof expected ${worldCount * 5} frames, got ${proof.frames?.length ?? 0}`);
      if ((proof.comparisons?.length ?? 0) !== 5) failures.push('style frame proof requires five same-moment comparison plans');
      for (const world of output.creativeWorldExploration?.worlds ?? []) {
        if (!proof.frames?.some((frame) => frame.worldId === world.id && frame.frameType === 'mobile')) failures.push(`style frame proof missing mobile frame for ${world.id}`);
      }
      if (proof.truth?.worldSelectedAutomatically === true) failures.push('style frame proof selected a world automatically');
      if (proof.truth?.humanVisualApproval === true) failures.push('style frame proof fabricated human visual approval');
    }
  }
