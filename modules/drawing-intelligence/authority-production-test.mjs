...
function materialSeam(intent) {
  ...
  if (retained(intent, 'domain-contrast')) {
    pieces.push(circle(4.25, 12, micro ? 1.65 : 2, 'fill="none"'));
...
function offsetThreshold(intent) {
  ...
  if (retained(intent, 'domain-contrast')) {
    pieces.push(circle(4.25, micro ? 9.25 : 9, micro ? 1.55 : 1.9, 'fill="none"'));
...
function inRange(value, min = 0, max = 24) {
  return Number.isFinite(value) && value >= min && value <= max;
}

export function inspectAuthoritySvgIntegrity(svg, { candidateId, targetSize } = {}) {
  const findings = [];
  const block = (code, message) => findings.push({ severity: 'BLOCKER', code, message });
  if (typeof svg !== 'string' || !svg.startsWith('<svg ')) block('authority-svg-root-invalid', 'SVG root is missing.');
  if (!svg.includes('viewBox="0 0 24 24"')) block('authority-svg-viewbox-invalid', 'Authority SVG must use canonical 0 0 24 24 viewBox.');
  if (!svg.includes(`width="${targetSize}"`) || !svg.includes(`height="${targetSize}"`)) block('authority-svg-size-invalid', 'SVG width/height must match target size.');
  if (!svg.includes('stroke="currentColor"')) block('authority-svg-currentcolor-missing', 'Authority SVG must inherit semantic presentation through currentColor.');
  if (/<image\b|data:image/i.test(svg)) block('authority-svg-raster-forbidden', 'Authority SVG cannot embed raster content.');
  if (/fill="(?:white|black|#[0-9a-f]{3,8}|rgb\(|hsl\()/i.test(svg)) block('authority-svg-raw-fill-forbidden', 'Authority SVG cannot contain surface-coupled or raw fill colors.');
  if (/stroke="(?:white|black|#[0-9a-f]{3,8}|rgb\(|hsl\()/i.test(svg)) block('authority-svg-raw-stroke-forbidden', 'Authority SVG cannot contain raw stroke colors.');
  if (candidateId && !svg.includes(`id="authority-${candidateId}-${targetSize}"`)) block('authority-svg-group-id-drift', 'Authority SVG stable group id drifted.');

  const shapeTags = [...svg.matchAll(/<(line|circle|rect)\b[^>]*>/g)].map((match) => ({ type: match[1], tag: match[0] }));
  if (!shapeTags.length) block('authority-svg-shapes-missing', 'Authority SVG must contain emitted vector shapes.');
  for (const shape of shapeTags) {
    if (shape.type === 'line') {
      const x1 = attr(shape.tag, 'x1'); const y1 = attr(shape.tag, 'y1'); const x2 = attr(shape.tag, 'x2'); const y2 = attr(shape.tag, 'y2');
      if (![x1, y1, x2, y2].every((value) => inRange(value))) block('authority-svg-line-bounds-invalid', `Line is outside canonical viewBox: ${shape.tag}`);
    } else if (shape.type === 'circle') {
      const cx = attr(shape.tag, 'cx'); const cy = attr(shape.tag, 'cy'); const r = attr(shape.tag, 'r');
      if (!Number.isFinite(r) || r <= 0 || !inRange(cx - r) || !inRange(cx + r) || !inRange(cy - r) || !inRange(cy + r)) block('authority-svg-circle-bounds-invalid', `Circle is outside canonical viewBox: ${shape.tag}`);
    } else if (shape.type === 'rect') {
      const x = attr(shape.tag, 'x'); const y = attr(shape.tag, 'y'); const width = attr(shape.tag, 'width'); const height = attr(shape.tag, 'height');
      if (!Number.isFinite(width) || width <= 0 || !Number.isFinite(height) || height <= 0 || !inRange(x) || !inRange(y) || !inRange(x + width) || !inRange(y + height)) block('authority-svg-rect-bounds-invalid', `Rect is outside canonical viewBox: ${shape.tag}`);
    }
  }
  ...
  return {
    ...
    findings,
    shapeCount: shapeTags.length,
    surfaceNeutral: !findings.some((item) => item.code === 'authority-svg-raw-fill-forbidden' || item.code === 'authority-svg-raw-stroke-forbidden')
  };
}
...
export function buildAuthorityVectorArtifact(intent) {
  ...
  if (vectorSpecValidation.status !== 'ready') throw new Error(`Authority vector spec failed validation for ${intent.candidateId}@${intent.targetSize}: ${JSON.stringify(vectorSpecValidation.findings)}`);
  ...
  const emittedSvgIntegrity = inspectAuthoritySvgIntegrity(svg, { candidateId: intent.candidateId, targetSize: intent.targetSize });
  if (emittedSvgIntegrity.status !== 'ready') throw new Error(`Authority emitted SVG integrity failed for ${intent.candidateId}@${intent.targetSize}: ${JSON.stringify(emittedSvgIntegrity.findings)}`);
