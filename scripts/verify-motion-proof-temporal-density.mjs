...
function denseBinding(submitted, independent, submittedAnchorTime, independentAnchorTime, durationSeconds) {
  ...
  return {
    verified: matchedSpan.ratio >= MIN_ACTIVE_SPAN_RATIO_BETWEEN_RECORDINGS
      ...
      && coverage.leftGap <= MAX_CONSECUTIVE_UNMATCHED
      && coverage.rightGap <= MAX_CONSECUTIVE_UNMATCHED
      && maxDrift <= MAX_NORMALIZED_PROGRESS_DRIFT
      && progressive(submitted)
      && progressive(independent),
    matches: coverage.matches.length,
    leftCoverage: coverage.leftCoverage,
    rightCoverage: coverage.rightCoverage,
    monotonicCoverage: coverage.monotonicCoverage,
