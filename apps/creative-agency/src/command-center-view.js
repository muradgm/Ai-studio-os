...
export function renderCommandCenterView({ stages = [], projectName = 'Project 001' } = {}) {
  ...
  <div class="app-shell command-center-app">
    <aside class="rail app-rail">
      <div class="agency-brand" title="Provisional application mark — not canonical brand master">
        <span class="ca-monogram">CA</span><span class="ca-wordmark">The Creative<br/>Agency</span>
      </div>
      <nav class="rail-nav" aria-label="Primary">
        <span class="rail-section-label">COMMAND</span>
        ${nav.map(([id, label]) => `<button type="button" class="rail-button ${id === 'command' ? 'active' : ''}" aria-label="${label}">${icon(id)}<span>${label}</span></button>`).join('')}
      </nav>
      <div class="rail-system-label">SYSTEM</div>
      <div class="rail-index">AI Studio OS · Production Proof</div>
    </aside>

    <main class="workspace approved-command-shell" id="command-center">
      ...
      <nav class="approved-pipeline project-spine" aria-label="Project stages">
        ${stages.map((stage, index) => `<button type="button" class="spine-step ${index === 0 ? 'active' : ''}" data-stage="${stage.id}" ${index === 0 ? 'data-release-probe="stage-brief"' : ''}><span class="stage-dot"></span><strong>${escapeHtml(stage.label)}</strong>${index < stages.length - 1 ? '<i></i>' : ''}</button>`).join('')}
      </nav>
