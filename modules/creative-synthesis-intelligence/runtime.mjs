...
const AUTHORITY_KEY = /(winner|recommend|select|approve|canonical|authorit|production|technicalplanning|score)/i;

function authorityClaims(object = {}) {
  const claims = [];
  for (const [key, value] of Object.entries(object && typeof object === 'object' ? object : {})) {
    if (key !== 'truth' && ((value === true && AUTHORITY_KEY.test(key)) || (typeof value === 'number' && /score/i.test(key)))) claims.push(key);
  }
  for (const [key, value] of Object.entries(object?.truth && typeof object.truth === 'object' ? object.truth : {})) {
    if ((value === true && AUTHORITY_KEY.test(key)) || (typeof value === 'number' && /score/i.test(key))) claims.push(`truth.${key}`);
  }
...
const CONTRADICTION_KEYS = Object.freeze(['id', 'statement']);
const SOURCE_RECEIPT_KEYS = Object.freeze([
  ...
  'transferHypothesisSnapshotFingerprint', 'transferBriefSnapshotFingerprint'
]);
...
const SOURCE_CANDIDATE_KEYS = Object.freeze([
  ...
  'transferCandidateSnapshotFingerprint', 'transferHypothesisSnapshotFingerprint',
  'transferBriefSnapshotFingerprint', 'candidate', 'truth'
]);
...
function sourceBindingFingerprint(binding = {}) {
  return fingerprintCreativeValue({
    ...
    sourceReceipts: (Array.isArray(binding.sourceReceipts) ? binding.sourceReceipts : [])
      .map((item) => ({
        sourceCandidateId: text(item?.sourceCandidateId),
        sourceProjectId: text(item?.sourceProjectId),
        sourceTargetDomain: text(item?.sourceTargetDomain),
        transferCandidateSnapshotFingerprint: text(item?.transferCandidateSnapshotFingerprint),
        transferHypothesisSnapshotFingerprint: text(item?.transferHypothesisSnapshotFingerprint),
        transferBriefSnapshotFingerprint: text(item?.transferBriefSnapshotFingerprint)
      }))
...
function buildSourceBinding(sources, projectId, targetDomain) {
  ...
  const candidateFingerprints = records.map((item) => item.receipt.transferCandidateSnapshotFingerprint).filter(Boolean);
  const duplicateIds = new Set(ids).size !== ids.length;
  const duplicateCandidates = new Set(candidateFingerprints).size !== candidateFingerprints.length;
  ...
  const allSourcesVerified = records.length >= 2
    && !duplicateIds
    && !duplicateCandidates
    && validReceipts
    && projectAligned
...
function canonicalBriefCore({ projectId, target, projectTruths, contradictions, constraints, sources } = {}) {
  ...
  const normalizedTruths = (Array.isArray(projectTruths) ? projectTruths : []).map(normalizeProjectTruth);
  const normalizedContradictions = (Array.isArray(contradictions) ? contradictions : []).map(normalizeContradiction);
  ...
  const sourceCandidates = sourceBinding.allSourcesVerified
    ? sourceBinding.records.map((item) => sourceProjection(item.source, (Array.isArray(sources) ? sources : []).indexOf(item.source)))
      .sort((left, right) => compareText(left.sourceCandidateId, right.sourceCandidateId))
    : [];
  const brief = {
    schema: 'ai-studio-os/creative-synthesis-brief@1',
    ...
    projectTruths: normalizedTruths,
...
export function reviewCreativeSynthesisBrief(brief = {}) {
  ...
  const contradictions = (Array.isArray(brief.contradictions) ? brief.contradictions : []).map(normalizeContradiction);
  ...
  rawTruths.forEach((raw, index) => {
    const unknown = unknownKeys(raw, PROJECT_TRUTH_KEYS);
    if (unknown.length || !sameValue(raw ?? {}, projectTruths[index])) findings.push(finding('blocker', 'creative-synthesis-project-truth-contract-drift', 'Project truths must use the canonical id/statement shape.', { index, unknownKeys: unknown }));
  });
  const truthIds = projectTruths.map((item) => item.id);
  if (!projectTruths.length) findings.push(finding('blocker', 'creative-synthesis-project-truth-missing', 'Creative Synthesis requires explicit project truth grounding.'));
  if (truthIds.some((id) => !id) || new Set(truthIds).size !== truthIds.length || projectTruths.some((item) => !item.statement)) findings.push(finding('blocker', 'creative-synthesis-project-truth-invalid', 'Project truths require unique IDs and non-empty statements.'));

  const rawContradictions = Array.isArray(brief.contradictions) ? brief.contradictions : [];
  rawContradictions.forEach((raw, index) => {
    const unknown = unknownKeys(raw, CONTRADICTION_KEYS);
    if (unknown.length || !sameValue(raw ?? {}, contradictions[index])) findings.push(finding('blocker', 'creative-synthesis-contradiction-contract-drift', 'Contradictions must use the canonical id/statement shape.', { index, unknownKeys: unknown }));
  });
  const contradictionIds = contradictions.map((item) => item.id);
  if (contradictions.length < 2) findings.push(finding('major', 'creative-synthesis-contradiction-mining-thin', 'Creative Synthesis should expose at least two productive project contradictions before combining sources.', { count: contradictions.length }));
  if (contradictionIds.some((id) => !id) || new Set(contradictionIds).size !== contradictionIds.length || contradictions.some((item) => !item.statement)) findings.push(finding('blocker', 'creative-synthesis-contradiction-invalid', 'Contradictions require unique IDs and non-empty statements.'));
  ...
  receipts.forEach((raw, index) => {
    ...
    if (unknown.length) findings.push(finding('blocker', 'creative-synthesis-source-receipt-shape-invalid', 'Synthesis source receipts may contain only canonical provenance fields.', { index, unknownKeys: unknown }));
    for (const key of ['transferCandidateSnapshotFingerprint', 'transferHypothesisSnapshotFingerprint', 'transferBriefSnapshotFingerprint']) {
  ...
  const receiptCandidates = receipts.map((item) => text(item?.transferCandidateSnapshotFingerprint));
  ...
  if (new Set(receiptCandidates).size !== receiptCandidates.length) findings.push(finding('blocker', 'creative-synthesis-duplicate-source-candidate', 'The same Transfer candidate cannot be counted multiple times as independent Synthesis evidence.'));
  if (!sameValue(sortedList(binding?.requestedSourceIds), [...receiptIds].sort(compareText))) findings.push(finding('blocker', 'creative-synthesis-requested-source-set-drift', 'Requested Synthesis source IDs must exactly match the bound source receipts.'));
  ...
  sourceCandidates.forEach((raw, index) => {
    ...
    if (candidateUnknown.length || !sameValue(raw?.candidate ?? {}, normalizeTransferCandidatePayload(raw?.candidate ?? {}))) findings.push(finding('blocker', 'creative-synthesis-source-candidate-payload-drift', 'Synthesis source candidate must preserve the canonical safe Transfer candidate projection.', { index, unknownKeys: candidateUnknown }));
  ...
  const computedFingerprint = briefFingerprint({ ...brief, target, projectTruths, contradictions, constraints });
...
export function reviewCreativeSynthesisBriefProvenance({ brief, sources } = {}) {
  ...
  if (rebuilt.sourceBinding.allSourcesVerified !== true) {
    const invalidSources = rebuilt.sourceBinding.records
      .filter((item) => item.review.reviewReady !== true)
      .map((item) => item.id);
    findings.push(finding('blocker', 'creative-synthesis-brief-provenance-source-not-verified', 'Synthesis Brief provenance requires every requested Transfer Candidate to independently pass Candidate → Hypothesis → Brief → Retrieval → Graph → Foundation verification.', { invalidSourceIds: invalidSources }));
  }
  ...
  if (rebuilt.sourceBinding.duplicateCandidates) findings.push(finding('blocker', 'creative-synthesis-brief-provenance-candidate-duplicate', 'Duplicate Transfer Candidate snapshots cannot manufacture source multiplicity.'));
...
export function buildCreativeSynthesisBrief({ projectId, target, projectTruths = [], contradictions = [], constraints = [], sources = [] } = {}) {
  ...
  if (sourceBinding.allSourcesVerified !== true) findings.push(finding('blocker', 'creative-synthesis-brief-source-provenance-blocked', 'Default Synthesis Brief construction requires every requested Transfer Candidate to be independently verified and project/target aligned. No partial source candidate content was emitted.'));
  const blockers = findings.filter((item) => item.severity === 'blocker');
  const majors = findings.filter((item) => item.severity === 'major');
  const reviewReady = blockers.length === 0 && majors.length === 0;
  const provenanceReady = reviewReady && sourceBinding.allSourcesVerified === true;
  return {
    ...brief,
    findings,
    pass: blockers.length === 0,
...
function hypothesisContentFingerprint(value = {}) {
  const normalized = normalizeHypothesis(value);
  return fingerprintCreativeValue({ ...normalized, id: null });
}
...
function sourceProbePhrases(brief = {}) {
  ...
  for (const source of Array.isArray(brief.sourceCandidates) ? brief.sourceCandidates : []) {
    ...
    phrases.push(
      candidate.transferClaim,
      candidate.causalBridge,
      candidate.targetConsequence,
      ...candidate.adaptationActions,
      ...candidate.adaptationRuleActions,
      ...candidate.copyRiskMitigations,
...
function synthesisCorpus(hypothesis = {}) {
  return [
    hypothesis.governingIdea,
    hypothesis.productiveTension,
    hypothesis.combinationMechanism,
    ...(hypothesis.sourceContributions ?? []).map((item) => item.contribution),
    ...(hypothesis.experientialConsequences ?? []),
    ...(hypothesis.antiGenericClaims ?? []),
    hypothesis.ownabilityRisk,
    hypothesis.competitorTransferTest?.question,
    hypothesis.competitorTransferTest?.failureCondition,
...
export function reviewCreativeSynthesisSet(synthesis = {}, { brief } = {}) {
  const findings = [];
  const briefReview = reviewCreativeSynthesisBrief(brief ?? {});
  const hypotheses = (Array.isArray(synthesis.hypotheses) ? synthesis.hypotheses : []).map(normalizeHypothesis);
  ...
  const contentFingerprints = hypotheses.map(hypothesisContentFingerprint);
  if (new Set(contentFingerprints).size !== contentFingerprints.length) findings.push(finding('blocker', 'creative-synthesis-hypothesis-payload-duplicate', 'Renaming the same Synthesis payload does not create a divergent hypothesis.'));
  ...
  rawHypotheses.forEach((raw, index) => {
    ...
    if (unknown.length || !sameValue(raw ?? {}, hypothesis)) findings.push(finding('blocker', 'creative-synthesis-hypothesis-contract-drift', 'Synthesis hypotheses must use the exact canonical shape and canonical ordering.', { hypothesisId: hypothesis.id, unknownKeys: unknown }));
    if (!STRATEGY_SET.has(hypothesis.strategy)) findings.push(finding('blocker', 'creative-synthesis-strategy-invalid', 'Synthesis hypothesis strategy must use one of the canonical structural combination strategies.', { hypothesisId: hypothesis.id, strategy: hypothesis.strategy }));
    ...
    if (!hypothesis.productiveTension) findings.push(finding('major', 'creative-synthesis-productive-tension-missing', 'Each Synthesis hypothesis should state the productive tension it preserves rather than averaging sources into a neutral blend.', { hypothesisId: hypothesis.id }));
    ...
    if (!hypothesis.ownabilityRisk) findings.push(finding('major', 'creative-synthesis-ownability-risk-missing', 'Each Synthesis hypothesis must name the main risk that the idea could become generic or competitor-transferable.', { hypothesisId: hypothesis.id }));
...
export function buildCreativeSynthesisSet({ brief, sources, hypotheses = [] } = {}) {
  ...
  const normalizedHypotheses = sourceReady
    ? (Array.isArray(hypotheses) ? hypotheses : []).map(normalizeHypothesis)
    : [];
  const synthesis = {
    schema: 'ai-studio-os/creative-synthesis-set@1',
