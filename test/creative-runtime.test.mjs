...
test('real image with photographic correction routes to retouch',()=>assert.equal(routeImageAsset({real:true,usable:true,representsRealProduct:true,needs:['crop','grade']}).action,'retouch'));
test('missing truthful product image requires capture',()=>assert.equal(routeImageAsset({real:false,usable:false,representsRealProduct:true,needs:[]}).action,'capture-required'));
test('unusable real truth-sensitive asset requires capture',()=>assert.equal(routeImageAsset({real:true,usable:false,representsRealProduct:false,needs:[]}).action,'capture-required'));
test('supporting synthetic imagery may be generated',()=>assert.equal(routeImageAsset({real:false,usable:false,representsRealProduct:false,needs:[]}).action,'generate-supporting'));
...
test('critical creative eval failure blocks approval despite strong average',()=>{
  const result=evaluateCreative({businessClarity:9,brandFit:9,distinctiveness:9,visualHierarchy:9,imageAuthenticity:4,motionPurpose:9,accessibility:9,aiGenericRisk:2});
...
test('creative runtime keeps design, image, and motion under one direction while world selection remains unresolved',()=>{
  ...
  assert.match(output.creativeDirection.directionStatement,/precise transformations/i);
  assert.equal(output.selectedCreativeWorld,null);
});

test('Du Bonheur benchmark reaches visual-world proof gate without fabricating a winner',()=>{
  const output=runCreativeRuntime(input);
  ...
  assert.equal(output.creativeWorldExploration.reviewReady,true);
  assert.equal(output.creativeWorldExploration.review.truth.humanSemanticDivergenceReviewed,false);
  assert.equal(output.selectedCreativeWorld,null);
});
