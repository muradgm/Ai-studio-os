...
export function runCreativeProductionRuntime(input) {
  ...
  // Legacy calibration is retained temporarily for diagnostics/regression only.
  // In canonical mode it is explicitly not allowed to author Creative Direction.
  ...
  let selectionAuthority = 'legacy-concept-selection';
  if (canonicalMode) {
    creativeDirection = structuredClone(input.creativeDirection ?? {});
    selectionAuthority = 'advisory-only';
  } else {
  ...
  creativeDirection.calibration = {
    ...
    selectedConceptId: canonicalMode ? null : selection.selectedId,
    legacyConceptSelectionAuthority: canonicalMode ? 'advisory-only' : 'authoritative'
  };
  ...
  if (canonicalMode && canonicalHandoff.pass !== true) {
    return {
      ...
      canonicalHandoff,
      selectionAuthority,
      legacyCalibration: { exploration, selection, authoritative: false },
      creativeDirection,
  ...
  return {
    ...
    selection,
    selectionAuthority,
    ...(canonicalHandoff ? { canonicalHandoff, legacyCalibration: { exploration, selection, authoritative: false } } : {}),
    creativeDirection,
    ...(typography ? { typography } : {}),
    design,
    image,
