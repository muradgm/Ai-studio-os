...
function reviewWorld(world, thesis) {
  ...
  const technologyText = [
    world.worldIdea,
    world.interpretationOfThesis,
    world.worldClass,
    world.narrativeModel,
    world.compositionModel
  ].join(' ');
...
export function reviewCreativeWorldExploration(exploration = {}) {
  ...
  if (worlds.length && (worlds.length < 3 || worlds.length > 5)) {
    findings.push(finding('major', 'creative-world-candidate-count-invalid', 'Creative World Exploration requires 3–5 authored worlds.', { count: worlds.length }));
  }
  if (!worlds.length) {
    findings.push(finding('major', 'creative-world-authored-worlds-required', 'No authored Creative Worlds were supplied. The runtime may return an authoring brief, but it cannot fabricate creative authorship.'));
  }
  ...
  for (let i = 0; i < worlds.length; i += 1) {
    for (let j = i + 1; j < worlds.length; j += 1) {
      ...
      if (!result.pass) {
        findings.push(finding('major', 'creative-world-cosmetic-variation', 'Two candidate worlds are too structurally similar; palette/type/layout changes do not count as genuine world divergence.', result));
      }
    }
  }
  ...
  const status = blockers.length
    ...
    : majors.length
      ? 'provisional'
      : 'ready-for-world-selection-review';
...
export function buildCreativeWorldExploration({ creativeThesis = null, authoredWorlds = [] } = {}) {
  const worlds = (Array.isArray(authoredWorlds) ? authoredWorlds : [])
    .map((candidate, index) => reviewWorld(normalizeWorld(candidate, creativeThesis, index), creativeThesis));
  ...
  const exploration = {
    ...
    selectedWorld: null,
    truth: {
      authoredWorldsSupplied: worlds.length > 0,
      humanWorldSelectionConfirmed: false,
...
export function selectCreativeWorld(exploration = {}, { worldId, humanConfirmed = false, rationale = '' } = {}) {
  ...
  if (exploration.reviewReady !== true) {
    findings.push(finding('blocker', 'creative-world-exploration-not-review-ready', 'A Creative World cannot be selected until the exploration set is structurally review-ready.'));
  }
  if (!id || !selected) {
    findings.push(finding('blocker', 'creative-world-selection-invalid', 'Creative World selection must reference one candidate in the current exploration.', { worldId: id || null }));
  }
  if (humanConfirmed !== true) {
    findings.push(finding('major', 'creative-world-human-selection-required', 'No automated score may select the Creative World. Explicit human/creative-director confirmation is required.'));
  }
  ...
  return {
    ...
    selectedWorld,
    status: confirmed ? 'selected-awaiting-style-frame-review' : exploration.status,
