...
async function startIndependentFrameCounter(context, page) {
  ...
  const { executionContextId } = await cdp.send('Page.createIsolatedWorld', {
    ...
    worldName: `motion-proof-reduced-verifier-${Date.now()}`,
...
async function verifyReducedMotionTarget(browser, target) {
  ...
  try {
    ...
    if (!(media.videoWidth > 0) || !(media.videoHeight > 0) || !(media.duration > 0) || media.readyState < 1) {
      ...
      const anchorTimes = [0.7, 0.5, 0.35, 0.22, 0.12, 0.05]
        .map((offset) => media.duration - offset)
        .filter((time) => time > 0.01 && time < media.duration);
      const anchors = await decodedVideoFramesAtTimes(page, '#proof-video', anchorTimes);
      const best = anchors.map((sample) => ({ ...sample, distance: visualDistance(captureSignature.pixels, sample.pixels) }))
        .sort((left, right) => left.distance.meanDelta - right.distance.meanDelta || left.distance.outlierShare - right.distance.outlierShare)[0];
      if (!best || !visuallyBound(best.distance)) findings.push({ code: 'motion-proof-independent-video-replay-mismatch', message: 'Reduced-motion WebM does not bind to the independently verified final state.' });
    }
  } catch (error) {
