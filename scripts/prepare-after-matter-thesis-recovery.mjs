...
if (packet.deliberation.reviewReady !== true || packet.proposedCreativeThesis.reviewReady !== true || packet.thesisAuthorityReview.reviewReady !== true || packet.truth.creativeWorldSelectionAuthorityCreated !== false) {
  throw new Error('After Matter Thesis recovery packet did not preserve reviewed Thesis authority without creating Creative World authority.');
}

await fs.writeFile(outputPath, `${JSON.stringify(packet, null, 2)}\n`);
console.log(JSON.stringify({ status: packet.status, outputPath: path.relative(root, outputPath).split(path.sep).join('/'), thesisFingerprint: packet.proposedCreativeThesisFingerprint, humanDecisionMode: packet.humanDecision.decision, humanApproved: packet.thesisAuthorityReview.authority?.humanApproved === true, geminiGenerationUsed: packet.truth.geminiGenerationUsed }, null, 2));
