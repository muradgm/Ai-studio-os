...
export function runCreativeRuntime(input) {
  ...
  const styleFrameProof = Array.isArray(input.styleFrameMoments) && input.styleFrameMoments.length
    ? buildStyleFrameProof({ exploration: worldExplorationBase, moments: input.styleFrameMoments })
    : null;
  ...
  const proofBlocked = styleFrameProof?.findings?.some((item) => item.severity === 'blocker') === true;
  ...
  let status;
  if (!creativeThesis.pass || worldBlocked || proofBlocked) status = 'blocked';
  else if (creativeThesis.reviewReady === false || creativeWorldExploration.reviewReady === false || styleFrameProof?.reviewReady === false) status = 'provisional';
  ...
  return {
    ...
    taskType: input.taskType,
    status,
    stages: ['product-understanding', 'inspiration', 'creative-thesis', 'creative-world', 'style-frame-proof', 'creative-direction', 'design', 'image', 'motion', 'creative-eval'],
    productUnderstanding,
    inspiration,
    creativeThesis,
...
export function validateBenchmark(output, expected) {
  ...
  if (expected.requireStyleFrameProof) {
    if (!output.styleFrameProof) failures.push('style frame proof is missing');
    else {
      if (output.styleFrameProof.reviewReady !== true) failures.push('style frame proof is not review-ready');
      if ((output.styleFrameProof.frames?.length ?? 0) !== (output.styleFrameProof.moments?.length ?? 0) * (output.creativeWorldExploration?.worlds?.length ?? 0)) failures.push('style frame proof coverage is incomplete');
      if (output.styleFrameProof.selection) failures.push('style frame proof fabricated a selected world');
      if (output.styleFrameProof.truth?.humanVisualApproval === true) failures.push('style frame proof fabricated human visual approval');
    }
  }
