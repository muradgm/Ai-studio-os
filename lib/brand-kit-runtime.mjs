...
export function runBrandKitRuntime(input = {}) {
  ...
  const typography = consumeTypographyContract(
    input.typographyContract ?? input.design?.typography?.consumption ?? input.brandDNA?.typography?.consumption ?? null,
    { surface: 'brand-kit' }
  );
  const brandDNAInput = structuredClone(input.brandDNA ?? {});
  if (typography.enabled) {
    brandDNAInput.typography = {
      ...(brandDNAInput.typography ?? {}),
      schema: typography.schema,
      roles: typography.roles,
      usage: typography.usage,
      application: typography.application,
      cssVariables: typography.cssVariables,
      integration: typography.integration
    };
  }
  const brandDNA = createBrandDNA(brandDNAInput);
...
export function validateBrandKitBenchmark(output = {}, expected = {}) {
  ...
  if (expected.typographyEnabled !== undefined && output.typography?.enabled !== expected.typographyEnabled) failures.push(`expected typography enabled=${expected.typographyEnabled}`);
  if (expected.typographyBodyFamily && output.typography?.roles?.body?.family !== expected.typographyBodyFamily) failures.push(`expected typography body ${expected.typographyBodyFamily}`);
