...
function hasCompleteCanonicalAuthorityBundle(input = {}) {
  const visualProofEvidence = input.visualProofEvidence ?? input.styleFrameProofEvidence;
  return Boolean(
    input.creativeThesisDeliberation
    && input.creativeThesis
    && input.selectedCreativeWorld
    && input.creativeWorldExploration
    && visualProofEvidence
    && input.creativeDirection
  );
}

function canonicalAuthorityRequested(input = {}) {
  // A thesis, world, or proof artifact may legitimately be supplied as context to
  // legacy callers. Enter canonical mode only on explicit intent or a complete
  // authority bundle so optional upstream context cannot silently change runtime semantics.
  return input.canonicalCreativeAuthority === true || hasCompleteCanonicalAuthorityBundle(input);
}

export function runCreativeProductionRuntime(input) {
  const canonicalMode = canonicalAuthorityRequested(input);
  ...
  if (!productUnderstanding.reviewReady) {
    return {
      ...
      findings: [{
        severity: 'blocker',
        code: 'creative-production-product-understanding-not-ready',
        message: 'Concept/world exploration is blocked until Product Understanding is review-ready.'
      }]
    };
  }
  ...
  if (canonicalMode) {
    // Canonical production does not execute the retired concept-selection path.
    // The reviewed Creative World is the sole world-level art-direction authority.
    creativeDirection = structuredClone(input.creativeDirection ?? {});
    selectionAuthority = 'canonical-creative-world';
  } else {
    // Temporary backwards-compatible legacy mode for existing benchmarks only.
    exploration = buildConceptExploration({
    ...
    selection = selectConcept({ exploration, ...(input.selection ?? {}) });
    ...
    selectionAuthority = 'legacy-concept-selection';
  }

  creativeDirection.calibration = {
    ...(creativeDirection.calibration ?? {}),
    productUnderstanding: productUnderstanding.projectId,
    designRead: designRead.memorableIdea,
    creativeDials: creativeDials.values,
    dialFingerprint: creativeDials.fingerprint,
    selectedConceptId: canonicalMode ? null : selection?.selectedId ?? null,
    legacyConceptSelectionAuthority: canonicalMode ? 'retired' : 'authoritative'
  };

  const typography = input.typography
    ? buildTypographySystem({
        ...input.typography,
        creativeThesis: input.typography.creativeThesis ?? input.creativeThesis ?? null,
        creativeWorld: input.typography.creativeWorld ?? input.selectedCreativeWorld ?? input.creativeWorld ?? null,
        catalog: input.fontCatalog ?? input.typography.catalog ?? []
      })
    : null;

  const canonicalHandoff = canonicalMode
  ...
  if (canonicalMode && canonicalHandoff.pass !== true) {
    return {
      id: input.id,
      taskType: input.taskType,
      version: '1.1',
      status: 'blocked',
      stages: ['product-understanding', 'canonical-creative-authority'],
      productUnderstanding,
      canonicalHandoff,
      selectionAuthority,
      creativeDirection,
      ...(typography ? { typography } : {}),
      findings: canonicalHandoff.findings,
      pass: false
    };
  }
  ...
  const calibrationReady = canonicalMode
    ? [productUnderstanding.reviewReady, inspirationReady, referenceSystem.pass, designRead.pass, creativeDials.pass, canonicalHandoff.pass].every(Boolean)
    : [productUnderstanding.reviewReady, inspirationReady, referenceSystem.pass, designRead.pass, creativeDials.pass, exploration?.pass, selection?.pass].every(Boolean);
  ...
  const stagePass = [
    productUnderstanding.reviewReady,
    calibrationReady,
    typographyReady,
    referenceSystem.pass,
    designRead.pass,
    creativeDials.pass,
    ...(canonicalMode ? [canonicalHandoff.pass] : [exploration?.pass, selection?.pass]),
    productionMode.pass,
    recipe.pass,
    gateway.pass,
    registry.pass,
  ...
  const stages = [
    'product-understanding', 'inspiration', 'reference-extraction', 'design-read', 'creative-dials',
    ...(canonicalMode ? ['canonical-creative-authority'] : ['explore', 'concept-selection']),
  ...
  return {
    ...
    designRead,
    creativeDials,
    ...(canonicalMode ? {} : { exploration, selection }),
    selectionAuthority,
...
export function validateCreativeProductionBenchmark(output, expected) {
  ...
  if (expected.requireProductUnderstanding && output.productUnderstanding?.reviewReady !== true) failures.push('product understanding is not review-ready');
  if (expected.selectedConceptId && output.selection?.selectedId !== expected.selectedConceptId) failures.push(`expected selected concept ${expected.selectedConceptId}, got ${output.selection?.selectedId}`);
  if (expected.recipeId && output.recipe?.recipeId !== expected.recipeId) failures.push(`expected recipe ${expected.recipeId}, got ${output.recipe?.recipeId}`);
