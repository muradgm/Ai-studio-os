...
export const CREATIVE_MOTION_DOGFOOD_CONDITIONS = Object.freeze([
  Object.freeze({
    id: 'A',
    profile: 'motion-v1-baseline',
    motionV2: false,
    knowledgeProfile: 'none',
    synthesis: false,
    directModelControl: false
  }),
  Object.freeze({
    id: 'B',
    profile: 'motion-v2-core-knowledge',
    motionV2: true,
    knowledgeProfile: 'qualified-core',
    synthesis: false,
    directModelControl: false
  }),
  Object.freeze({
    id: 'C',
    profile: 'motion-v2-full-knowledge',
    motionV2: true,
    knowledgeProfile: 'full-qualified-motion-corpus',
    synthesis: false,
    directModelControl: false
  }),
  Object.freeze({
    id: 'D',
    profile: 'motion-v2-full-knowledge-plus-synthesis',
    motionV2: true,
    knowledgeProfile: 'full-qualified-motion-corpus',
    synthesis: true,
    directModelControl: false
...
export const CREATIVE_MOTION_DOGFOOD_RATINGS = Object.freeze([
  'weak',
  'mixed',
  'strong',
  'exceptional'
...
function compareText(a, b) { return String(a).localeCompare(String(b), 'en', { sensitivity: 'variant' }); }
...
function validateBrief(brief, projectId, findings) {
  if (!brief.projectId || brief.projectId !== projectId) {
    findings.push(finding('blocker', 'dogfood-brief-project-invalid', 'Dogfood brief must bind the same explicit project identity as the experiment.'));
  }
...
function validateConditions(conditions, findings) {
  ...
  if (new Set(ids).size !== ids.length || ids.some((id) => !CONDITION_IDS.has(id))) {
    findings.push(finding('blocker', 'dogfood-condition-id-invalid', 'Dogfood condition IDs must be exactly A, B, C, D and E.', { ids }));
  }
  for (const condition of conditions) {
    const expected = CONDITION_PROFILES.get(condition.id);
    if (!expected || !sameValue(condition, expected)) {
      findings.push(finding('blocker', 'dogfood-condition-profile-drift', 'Dogfood condition definitions are locked so the ablation cannot be relabelled after results are known.', { conditionId: condition.id || null }));
    }
  }
}
...
function validateTrials(trials, experiment, findings) {
  ...
  if (trialIds.some((id) => !id) || new Set(trialIds).size !== trialIds.length) {
    findings.push(finding('blocker', 'dogfood-trial-id-invalid', 'Every dogfood trial requires a unique stable trial ID.'));
  }

  const evidenceRefs = trials.map((trial) => trial.evidenceBundleRef).filter(Boolean);
  if (evidenceRefs.length !== trials.length || new Set(evidenceRefs).size !== evidenceRefs.length) {
    findings.push(finding('blocker', 'dogfood-evidence-bundle-invalid', 'Every trial requires a unique evidence bundle reference.'));
  }

  const runtimeRefs = trials.map((trial) => trial.runtimeTraceRef).filter(Boolean);
  if (runtimeRefs.length !== trials.length || new Set(runtimeRefs).size !== runtimeRefs.length) {
    findings.push(finding('blocker', 'dogfood-runtime-trace-invalid', 'Every trial requires a unique runtime trace reference.'));
  }

  const expectedBriefFingerprint = experiment.briefFingerprint;
  ...
  for (const conditionId of CONDITION_IDS) {
    ...
    if (group.length !== 3 || !sameValue(replicates, [1, 2, 3])) {
      findings.push(finding('blocker', 'dogfood-condition-replicates-invalid', 'Every condition requires replicates 1, 2 and 3 exactly once.', { conditionId, replicates }));
    }
  }
  ...
  for (const trial of trials) {
    ...
    if (trial.briefFingerprint !== expectedBriefFingerprint) findings.push(finding('blocker', 'dogfood-trial-brief-drift', 'Every dogfood trial must bind the exact same brief snapshot.', { trialId: trial.trialId }));
    ...
    if (trial.generationBudget.maxGenerationAttempts === null || trial.generationBudget.tokenBudget === null || trial.generationBudget.wallClockSeconds === null || !trial.generationBudget.modelPolicyId || !trial.generationBudget.temperaturePolicyId) {
      findings.push(finding('blocker', 'dogfood-budget-incomplete', 'Each trial requires explicit generation, token, time, model and temperature policy budgets.', { trialId: trial.trialId }));
    }
    if (trial.hypothesisCount < 3) findings.push(finding('blocker', 'dogfood-hypothesis-count-thin', 'Each trial must expose at least three serious motion hypotheses.', { trialId: trial.trialId, count: trial.hypothesisCount }));
    if (trial.temporalStudyCount < 15) findings.push(finding('blocker', 'dogfood-temporal-proof-thin', 'Each trial must expose the full temporal comparison surface, including mobile and reduced motion.', { trialId: trial.trialId, count: trial.temporalStudyCount }));
...
export function buildCreativeMotionCapabilityDogfood({ experimentId, projectId, brief, conditions = canonicalConditionSet(), trials = [], blindSeed = '' } = {}) {
  ...
  const briefFingerprint = fingerprintCreativeValue(normalizedBrief);
  const normalizedConditions = (Array.isArray(conditions) ? conditions : []).map(conditionShape);
  const normalizedTrials = (Array.isArray(trials) ? trials : []).map(normalizeTrial);
  const experiment = {
    schema: 'ai-studio-os/creative-motion-capability-dogfood@1',
  ...
  return {
    ...
    truth: { ...experiment.truth, ...review.truth }
  };
}
...
export function reviewCreativeMotionCapabilityDogfood(experiment = {}, { blindSeed = '' } = {}) {
  ...
  for (const [key, expected] of Object.entries(experimentSnapshotCore(experiment).truth)) {
...
function blindIdFor(experiment, trial, blindSeed) {
  const digest = fingerprintCreativeValue({
...
export function buildCreativeMotionBlindReviewPacket(experiment = {}, { blindSeed = '' } = {}) {
  ...
  if (!review.reviewReady) {
    return {
      ...
      status: 'blocked',
      truth: {
        conditionIdentityHiddenFromReviewerPacket: true,
        ratingsAreDiagnosticNotAuthority: true,
        noAutomaticWinner: true,
        noCreativeDirectionSelected: true,
        noProductionAuthority: true
  ...
  const trials = experiment.trials.map(normalizeTrial);
  const mapped = trials.map((trial) => ({
    trial,
    blindId: blindIdFor(experiment, trial, blindSeed),
    orderKey: fingerprintCreativeValue({ experimentId: experiment.experimentId, trialId: trial.trialId, blindSeed: text(blindSeed), purpose: 'review-order' })
  })).sort((a, b) => compareText(a.orderKey, b.orderKey));

  const candidates = mapped.map(({ trial, blindId }) => ({
    blindId,
  ...
  const unblindingMap = mapped.map(({ trial, blindId }) => ({
    blindId,
    trialId: trial.trialId,
  ...
  return {
    ...
    truth: {
      ...
      unblindingRequiresSeparateMap: true,
      ratingsAreDiagnosticNotAuthority: true,
      noAutomaticWinner: true,
      noCreativeDirectionSelected: true,
      noProductionAuthority: true
...
function normalizeDimensionReview(value = {}) {
  return {
    dimensionId: text(value?.dimensionId),
    rating: text(value?.rating).toLowerCase(),
...
function normalizeCandidateReview(value = {}) {
  return {
    blindId: text(value?.blindId),
    dimensions: (Array.isArray(value?.dimensions) ? value.dimensions : []).map(normalizeDimensionReview)
  };
}
...
export function reviewCreativeMotionDogfoodResults(packet = {}, { reviewers = [], humanDecision = null } = {}) {
  const findings = [];
  const candidates = Array.isArray(packet?.candidates) ? packet.candidates : [];
  const unblindingMap = Array.isArray(packet?.unblindingMap) ? packet.unblindingMap : [];
  const blindIds = new Set(candidates.map((item) => text(item.blindId)));
  const mappingByBlindId = new Map(unblindingMap.map((item) => [text(item.blindId), item]));
  const normalizedReviewers = (Array.isArray(reviewers) ? reviewers : []).map(normalizeReviewer);

  if (packet?.schema !== 'ai-studio-os/creative-motion-blind-review-packet@1' || packet?.reviewReady !== true) findings.push(finding('blocker', 'dogfood-review-packet-invalid', 'Dogfood results require a review-ready blind packet.'));
  if (candidates.length !== 15 || unblindingMap.length !== 15) findings.push(finding('blocker', 'dogfood-review-packet-cardinality-invalid', 'Blind packet and unblinding map must each contain all 15 trials.'));
  ...
  for (const reviewer of normalizedReviewers) {
    ...
    for (const candidateReview of reviewer.candidateReviews) {
      ...
      if (dimensions.length !== CREATIVE_MOTION_DOGFOOD_DIMENSIONS.length || new Set(dimensionIds).size !== dimensionIds.length || dimensionIds.some((id) => !DIMENSION_IDS.has(id))) {
        findings.push(finding('blocker', 'dogfood-review-dimensions-invalid', 'Every candidate review must cover every canonical quality dimension exactly once.', { reviewerId: reviewer.reviewerId, blindId: candidateReview.blindId }));
      }
  ...
  for (const reviewer of normalizedReviewers) {
    for (const candidateReview of reviewer.candidateReviews) {
      const mapping = mappingByBlindId.get(candidateReview.blindId);
      if (!mapping) continue;
      for (const dimension of candidateReview.dimensions) {
        if (!RATING_IDS.has(dimension.rating)) continue;
        const key = `${mapping.conditionId}:${dimension.dimensionId}`;
    ...
    if (reviewer.topChoiceBlindId) {
      const mapping = mappingByBlindId.get(reviewer.topChoiceBlindId);
      if (mapping) topChoiceCounts.set(mapping.conditionId, (topChoiceCounts.get(mapping.conditionId) ?? 0) + 1);
    }
  }
  ...
  const conditionSummaries = CREATIVE_MOTION_DOGFOOD_CONDITIONS.map((condition) => ({
    ...
    dimensionDiagnostics: CREATIVE_MOTION_DOGFOOD_DIMENSIONS.map((dimensionId) => ({
      dimensionId,
      diagnosticMean: rounded(average(conditionDimensions.get(`${condition.id}:${dimensionId}`) ?? []))
    })),
  ...
  if (humanDecision !== null && typeof humanDecision === 'object') {
    normalizedDecision = {
      outcome: text(humanDecision?.outcome),
      rationale: text(humanDecision?.rationale),
      evidenceRef: text(humanDecision?.evidenceRef)
    };
